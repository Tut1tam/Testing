...
def equation():
    print(
        "Введите коэффициенты(A,B,C,D) Вашего уравнения вида - A*x^3 + B*x^2 + C*x + D = 0\n"
    )
    ...
    match type:
        case "Линейное":
            print(
                f"Решение вашего линейного уравнения {c}*x + {d} = 0\nX = {equation.calc_lin()}"
            )
        case "Квадратное":
            print(
                f"Решение вашего квадратного уравнения {b}*x^2 + {c}*x + {d} = 0\nX = {equation.calc_quad()}"
            )
        case _:
            print(
                f"К сожалению, мы пока не умеем решать такие уравнения {a}*x^3 + {b}*x^2 + {c}*x + {d} = 0\n"
            )
...
def integral():
    print(
        f"Введите N вашей функции f(x + N) интеграла вида: ∫f(x)dx\nВведите границы интегрирования x1, x0"
    )
    n, x1, x0 = input().split(" ")
